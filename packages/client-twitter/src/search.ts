...
export class TwitterSearchClient {
    ...
    async start() {
        ...
        const address = await this.LaunchToken({
            name: "SharmaToken",
            ticker: "ABHS",
            description:
                "SharmaToken is a community-driven cryptocurrency inspired by Abhishek Sharma. It aims to support artistic and creative projects, offering a platform for fans and creators to collaborate and fund innovative ideas, celebrating Sharma's influence and passion for creativity.",
            imagePrompt:
                "A sleek and modern logo featuring the initials 'AS' intertwined with a digital currency symbol, set against a vibrant, artistic background symbolizing creativity and collaboration.",
            sourceTrend: "Abhishek Sharma",
        });

        console.log("launched token", address);
...
`;
            ...
            const tokenInfo: TokenInfo = response.object as TokenInfo;

            try {
                elizaLogger.log("response", tokenInfo);
    ...
    private async LaunchToken(tokenInfo: TokenInfo) {
        try {
            ...
            const solanaAgentKit = new SolanaAgentKit(solanaPrivatekey, rpc, {
                OPENAI_API_KEY: openAIKey,
            });

            elizaLogger.log("Done solana agent kit...");
            ...
            elizaLogger.error("Error launching token", e?.message);
        }
    }
